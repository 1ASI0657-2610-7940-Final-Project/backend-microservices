...
public class MarketplaceRepositoryAdapter implements ServiceOfferingRepositoryPort, CategoryRepositoryPort, MediaRepositoryPort {
    ...
    public ServiceMedia save(ServiceMedia m){ var e=new ServiceMediaEntity(); e.id=m.id();e.serviceId=m.serviceId();e.publicUrl=m.url();e.mediaType=m.type();e.isPrimary=m.primary();e.bucket=m.bucket();e.path=m.path();e.contentType=m.contentType();e.sizeBytes=m.sizeBytes();e.createdAt=m.createdAt(); var s=mediaRepo.save(e); return new ServiceMedia(s.id,s.serviceId,s.publicUrl,s.mediaType,s.isPrimary,s.bucket,s.path,s.contentType,s.sizeBytes,s.createdAt); }
    public List<ServiceMedia> findByServiceId(UUID sid){ return mediaRepo.findByServiceId(sid).stream().map(x->new ServiceMedia(x.id,x.serviceId,x.publicUrl,x.mediaType,x.isPrimary,x.bucket,x.path,x.contentType,x.sizeBytes,x.createdAt)).toList(); }
    public Optional<ServiceMedia> findMediaById(UUID id){ return mediaRepo.findById(id).map(x->new ServiceMedia(x.id,x.serviceId,x.publicUrl,x.mediaType,x.isPrimary,x.bucket,x.path,x.contentType,x.sizeBytes,x.createdAt)); }
